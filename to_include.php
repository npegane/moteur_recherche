<?php
    ...
    function getDocWords($doc_id){
        ...
        $mysqli->query("SET NAMES UTF8;");
        $words = [];
        if ($stmt = $mysqli->prepare("SELECT id_mot FROM Occurence WHERE id_document = ? AND nb_occurence > 15")) {
            $stmt->bind_param("i", $doc_id);
            $stmt->execute();
            $stmt->bind_result($id_mot);
            // var_dump($id_mot);
            while($stmt->fetch()){
                if ($stmt2 = $mysqli->prepare("SELECT nom FROM Mot WHERE id = ?")) {
                    $stmt2->bind_param("i", $id_mot);
                    $stmt2->execute();
                    $stmt2->bind_result($wordName);
                    // var_dump($words);
